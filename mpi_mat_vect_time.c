...
#  endif
   ...
   FILE *tserial;
   if((tserial = fopen("Tserial.bin","rb"))==NULL){
       printf("Error al abrir el archivo");
       exit(1);
   }
   double timeS,poss;
   poss = log(n) / log(2.0);
   fseek(tserial,(poss-9.0)*sizeof(double),SEEK_CUR);
   fread(&timeS,sizeof(double),1,tserial);
...
#  endif
   ...
   double SpeedU,Efficience;

   elapsed = elapsed*1000.0;
   SpeedU = timeS/elapsed;
   Efficience = SpeedU/comm_sz;
   if (my_rank == 0)
       //%e to %f
      printf("Serial Time = %f \nElapsed time = %f\nSpeedup = %f\nEfficience %f\n", timeS,elapsed,SpeedU,Efficience);
